...
class Site:
  ...
  '''
    ...
    def template_parse(self):
        tags = [('{{NAV_ELEMENT}}', '{{/NAV_ELEMENT}}'), ('{{SUB_NAV}}'
                , '{{/SUB_NAV}}'), ('{{SUB_NAV_ELEMENT}}',
                '{{/SUB_NAV_ELEMENT}}')]
        template_formats = {}
    ...
    # loop over once to get the formats
        ...
        for (open_tag, close_tag) in tags:
            template_formats[open_tag] = \
                self.template[self.template.find(open_tag)
                + len(open_tag):self.template.find(close_tag)]
    ...
    # loop over again to get a cleaned form
        ...
        for (open_tag, close_tag) in tags:
            if open_tag in self.template:
                self.template = \
                    self.template[:self.template.find(open_tag)] \
                    + self.template[self.template.find(close_tag)
                    + len(close_tag):]
    ...
    def run(self, request):
        ...
        if request in self.pages:
            ...
            if self.pages[request].startswith('http') \
                or not self.pages[request].endswith('.html'):
...
'''
            ...
            for (category, elements) in self.categories.iteritems():
                if request in elements:
                    ...
                    if elements[request].startswith('http') \
                        or not elements[request].endswith('.html'):
    ...
    # set up our imports
        ...
        for an_import in self.imports:
            if an_import.endswith('.css'):
                import_statements += \
                    '<link rel="stylesheet" type="text/css" href="{link}" />'.format(link=an_import)
            elif an_import.endswith('.js'):
                import_statements += \
                    '<script src="{link}"></script>'.format(link=an_import)

    # set up our navigation

        nav_code = ''
        for nav_element in self.pages.keys():
            nav_code += template_formats['{{NAV_ELEMENT}}'
                    ].format(URL='?req=' + nav_element.replace(' ', '_'
                             ), TITLE=nav_element)
        ...
        for (category, elements) in self.categories.iteritems():
            ...
            for (nav_element, nav_element_url) in elements.iteritems():
                sub_nav_holder += template_formats['{{SUB_NAV_ELEMENT}}'
                        ].format(URL='?req=' + nav_element.replace(' ',
                                 '_'), TITLE=nav_element)
            category_format = \
                category_format[:category_format.find('{{SUB_NAV_ELEMENT'
                    )] \
                + category_format[category_format.find('{{/SUB_NAV_ELEMENT}}'
                    ) + 20:]
            nav_code += \
                category_format.format(SUB_NAV_ELEMENT_SPOT=sub_nav_holder)
...
''' \
            + self.template.format(IMPORTS=import_statements,
                                   NAV=nav_code, CONTENT=page_content,
                                   PAGE_TITLE=request,
